...
template <size_t S>
static inline void SetBitsFromValue(std::bitset<S> &set, size_t bit_start_index,
                                    size_t bit_end_index, uint32_t value, size_t value_bit_offset)
{
  if(bit_start_index > bit_end_index)
    return;
  if(bit_start_index >= S)
    return;
  if((uint64_t{1} << (value_bit_offset + bit_end_index - bit_start_index)) >
